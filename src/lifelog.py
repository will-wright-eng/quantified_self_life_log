...
class life_log(object):
    ...
    def __init__(self,
        ...
        if auto_run:
            self.run_analysis(self.spreadsheet_id, self.sample_range,
                              self.mapping_table, self.grouping)

    def run_analysis(self, spreadsheet_id, sample_range, mapping_table,
                     grouping):
        '''docstring for run_analysis function'''
        df = self.return_dataframe_from_sheet(spreadsheet_id, sample_range)
        df_mapping_table = self.return_dataframe_from_sheet(
            spreadsheet_id, mapping_table)
    ...
    def clean_dataframe(self, df, df_mapping_table, cols, grouping):
        ...
        for i in range(len(df_mapping_table)):
            model_dict[df_mapping_table[map_from].
                       iloc[i]] = df_mapping_table[map_to].iloc[i]
